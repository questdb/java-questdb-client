...
public class DirectByteSink implements DirectByteSequence, BorrowableAsNativeByteSink, QuietCloseable, Mutable {
    ...
    public long ensureCapacity(long required) {
        ...
        if (p == 0) {
            if (getImplOverflow()) {
                ...
                throw CairoException.nonCritical().put("could not allocate DirectByteSink [required=").put(required).put(']');
            }
        }
    ...
    private void inflate() {
        ...
        if (impl == 0) {
            throw CairoException.nonCritical().put("could not allocate direct byte sink [maxCapacity=").put(initialCapacity).put(']');
        }
