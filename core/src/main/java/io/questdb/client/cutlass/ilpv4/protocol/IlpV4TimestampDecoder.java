 ...
 * </pre>
 */
public final class IlpV4TimestampDecoder {
    ...
    /**
     * Decodes timestamp column data from native memory.
     *
     * @param sourceAddress source address in native memory
     * @param sourceLength  length of source data in bytes
     * @param rowCount      number of rows to decode
     * @param nullable      whether the column is nullable
     * @param sink          sink to receive decoded values
     * @return number of bytes consumed
     */
    public int decode(long sourceAddress, int sourceLength, int rowCount, boolean nullable, ColumnSink sink) {
        if (rowCount == 0) {
            return 0;
        }
        ...
        if (nullable) {
            ...
            if (offset + nullBitmapSize > sourceLength) {
                throw new IllegalArgumentException("insufficient data for null bitmap");
            }
        ...
        if (offset + 1 > sourceLength) {
            throw new IllegalArgumentException("insufficient data for encoding flag");
        }
        ...
        if (encoding == ENCODING_UNCOMPRESSED) {
            ...
            throw new IllegalArgumentException("unknown timestamp encoding: " + encoding);
        }
    ...
    private int decodeUncompressed(long sourceAddress, int sourceLength, int offset, int rowCount,
                                   boolean nullable, long nullBitmapAddress, ColumnSink sink) {
        ...
        if (offset + valuesSize > sourceLength) {
            throw new IllegalArgumentException("insufficient data for uncompressed timestamps");
        }
    ...
    private int decodeGorilla(long sourceAddress, int sourceLength, int offset, int rowCount,
        ...
        if (offset + 8 > sourceLength) {
            throw new IllegalArgumentException("insufficient data for first timestamp");
        }
        ...
        if (offset + 8 > sourceLength) {
            throw new IllegalArgumentException("insufficient data for second timestamp");
        }
    ...
    /**
     * Returns the expected size for decoding.
     *
    ...
    /**
     * Sink interface for receiving decoded column values.
     */
