...
public class QwpSchemaBindingStringDecimalTest {
    ...
    public void testCorpusUsesExactTargetDecimalWire() throws Exception {
        assertMemoryLeak(() -> {
            InputStream stream = QwpSchemaBindingStringDecimalTest.class.getResourceAsStream(CORPUS);
    ...
    public void testDuplicateNullOmissionAndWholeRowRollback() throws Exception {
        assertMemoryLeak(() -> {
            ...
            try (QwpWebSocketEncoder encoder = new QwpWebSocketEncoder();
                ...
                binding.stringColumn("value", "1.0").stringColumn("value", "invalid");
                buffer.nextRow();

                binding.stringColumn("only_b", "2.0");
                assertReason(LineSenderSchemaException.Reason.INVALID_VALUE,
                        () -> binding.stringColumn("value", "100.0"));
                buffer.cancelCurrentRow();
                buffer.rollbackUncommittedColumns();

                binding.stringColumn("value", "-2.5");
                buffer.nextRow();
                binding.stringColumn("value", null).stringColumn("value", "invalid");
                buffer.nextRow();
                buffer.nextRow();
    ...
    private static byte wireType(String target) {
        if ("DECIMAL128".equals(target)) return QwpConstants.TYPE_DECIMAL128;
        if ("DECIMAL256".equals(target)) return QwpConstants.TYPE_DECIMAL256;
        return QwpConstants.TYPE_DECIMAL64;
    }

    private static int wireLongCount(String target) {
        if ("DECIMAL128".equals(target)) return 2;
        if ("DECIMAL256".equals(target)) return 4;
        return 1;
    }
    ...
    private static final class Reader {
        ...
        private String string() {
            ...
            for (int i = 0; i < length; i++) bytes[i] = (byte) u8();
    ...
    private static final class Vector {
        ...
        private Vector(String[] fields) {
            ...
            for (int i = 0; i < limbs; i++) expectedLimbs[i] = parseHex(fields[6 + i]);
            for (int i = limbs; i < 4; i++) Assert.assertEquals("-", fields[6 + i]);
            if (outcome.equals("INVALID")) {
                Assert.assertEquals("-", fields[10]);
            } else if (outcome.equals("NULL")) {
                Assert.assertEquals("NULL", fields[10]);
            } else {
                Assert.assertNotEquals("-", fields[10]);
                Assert.assertNotEquals("NULL", fields[10]);
            }
        }
