...
public class QwpSchemaProtocolTest {
    ...
    @Test
    public void testGetColumnIndexIsCaseInsensitive() {
        QwpSchemaResponse known = decode(knownResponse(1, 1, 1, 1,
                column("Id", 5, new byte[0]),
                column("métric🚀", 6, new byte[0])));
        Assert.assertEquals(0, known.getColumnIndex("id"));
        Assert.assertEquals(0, known.getColumnIndex("ID"));
        Assert.assertEquals(1, known.getColumnIndex("MÉTRIC🚀"));
        Assert.assertEquals(-1, known.getColumnIndex("absent"));

        QwpSchemaResponse missing = decode(resultResponse(1, QwpSchemaProtocol.RESULT_MISSING));
        Assert.assertEquals(-1, missing.getColumnIndex("id"));
    }
