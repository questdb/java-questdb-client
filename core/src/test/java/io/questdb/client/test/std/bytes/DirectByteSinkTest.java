...
package io.questdb.client.test.std.bytes;

import io.questdb.client.cairo.CairoException;
import io.questdb.client.std.MemoryTag;
import io.questdb.client.std.Unsafe;
import io.questdb.client.std.bytes.ByteSequence;
...
public class DirectByteSinkTest {
    ...
    public void testBasics() {
        ...
        final long memUsed0 = Unsafe.getMemUsedByTag(MemoryTag.NATIVE_DEFAULT);
        ...
        final Supplier<Long> getMemUsed = () -> Unsafe.getMemUsedByTag(MemoryTag.NATIVE_DEFAULT) - memUsed0;

        try (DirectByteSink sink = new DirectByteSink(0, MemoryTag.NATIVE_DEFAULT)) {
            ...
            try (DirectByteSink other = new DirectByteSink(32, MemoryTag.NATIVE_DEFAULT)) {
    ...
    public void testBorrowNativeByteSink() {
        try (DirectByteSink sink = new DirectByteSink(16, MemoryTag.NATIVE_DEFAULT)) {
            final long ptr = sink.ptr();
            Assert.assertNotEquals(0, ptr);
            Assert.assertEquals(32, sink.allocatedCapacity());
            sink.put((byte) 'a');
    ...
    public void testOver2GiB() {
        try (
                DirectByteSink oneMegChunk = new DirectByteSink(32, MemoryTag.NATIVE_DEFAULT);
                DirectByteSink dbs = new DirectByteSink(32, MemoryTag.NATIVE_DEFAULT)
        ) {
