...
package io.questdb.client.test.cutlass.line.interop;
...
public class ClientInteropTest {
    ...
    public void testInterop() throws Exception {
        String pp = TestUtils.getTestResourcePath("/io/questdb/client/test/cutlass/line/interop/ilp-client-interop-test.json");
        final long memSize = 1024 * 1024;

        ByteChannel channel = new ByteChannel();
        try (JsonLexer lexer = new JsonLexer(1024, 1024);
             Sender sender = new LineTcpSenderV3(channel, 1024, 127);
                            ...
                            try {
                                switch (columnType) {
                                    ...
                                    case ColumnType.DECIMAL:
                                        try {
                                            sender.decimalColumn(name, Chars.toString(tag));
                                        } catch (LineSenderException e) {
                                            encounteredError = true;
                                        }
                                        break;
